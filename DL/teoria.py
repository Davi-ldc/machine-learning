...
rede_neural.add(Dense(units=50, activation='relu', kernel_initializer='glorot_uniform', input_dim = 30 )) #primeira camada oculta 
...
#2 camada oculta
rede_neural.add(Dense(units=50, activation='relu', kernel_initializer='glorot_uniform'))
...
rede_neural.fit(variaveis_previsoras_treinamento, classes_treinamento, epochs=1000, batch_size=1000000, callbacks=[tensorboard])
...
erro, pontuação = rede_neural.evaluate(variaveis_previsoras_teste, classes_teste)
print(pontuação)
print(erro)

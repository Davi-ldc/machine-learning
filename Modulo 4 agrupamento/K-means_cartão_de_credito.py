...
kmeans_final = KMeans(n_clusters=4, random_state=1)
previsoes = kmeans.fit_predict(variaveis_previsoras)# treina e retorna as previsoes
...
grafico_kmeans = px.scatter(x=variaveis_previsoras_q_da_pra_frz_grafico[:, 0], y=variaveis_previsoras_q_da_pra_frz_grafico[:, 1], color=previsoes)
grafico_kmeans.show()
